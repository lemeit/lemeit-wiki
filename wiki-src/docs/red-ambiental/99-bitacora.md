...
## Septiembre 2026 — Administración de visitas, enlaces a la wiki y rediseño de Historial/Gráfico en Aire Saladillo

- **Administración de visitas (Aire Saladillo)**: las consultas administrativas del Worker (conteo/detalle de visitas al dashboard) pasaron de clave por query param a header `X-Admin-Key`, para no dejar la clave en logs de acceso ni en historiales de navegador. Se sumó `scripts/ver-visitas.ps1`, un script de PowerShell para consultarlas desde la consola sin necesitar un navegador — motivado porque en Windows `curl` es un alias de `Invoke-WebRequest`, con una sintaxis de headers distinta a la del `curl` real.
- **Enlaces cruzados a la wiki**: el footer compartido (`lemeit-common.js`, usado por los tres portales) y la sección "Acerca de" de cada uno pasan a enlazar a [wiki.lemeit.ar](https://wiki.lemeit.ar) — hasta ahora la wiki no se mencionaba desde ningún portal.
...
- **Historial — reporte configurable y export PDF**: se agregó filtro de rango de fechas, filtro de parámetros (con checkboxes) y, más adelante en la misma tanda, columnas de PM1.0 y PM10 (ambos proveedores ya las reportaban, no se mostraban). Se sumó una vista "Reporte" en pantalla —texto plano monoespaciado estilo resumen climatológico NOAA (inspirada en chajarialdia.com.ar), con fondo papel fijo independiente del tema del sitio— y luego exportación real a PDF con jsPDF (fuente Courier incorporada, sin embeber archivos de fuente), con respaldo automático a jsDelivr si cdnjs no carga (se vio en producción: un bloqueador de contenido impedía cargar el script y tiraba `window.jspdf` indefinido; ahora además hay un aviso al usuario en vez de un error sin manejar).
- **Simplificación de Historial**: la vista "Reporte" en pantalla, tras uso real, resultaba redundante — el PDF ya se veía siempre en blanco/formato reporte, así que tener además un toggle Clásica/Reporte en pantalla (con el fondo invertido respecto de lo esperado) sumaba una opción de más. Se sacó el toggle: la tabla en pantalla usa siempre el formato normal del sitio, y el estilo reporte quedó reservado al PDF. Se agregó `accent-color` a los checkboxes del sitio (el azul por defecto del navegador desentonaba con la paleta), y se cambió "Estación" por "Sitio" en el PDF y demás lugares que nombran la ubicación de un sensor. El PDF, además de descargarse, ahora también se abre en una pestaña nueva (blob URL) dentro del mismo click, para poder verlo sin ir a buscar el archivo descargado.
- **Gráfico — comparación de sitios**: nuevo modo ("Comparar sitios") donde se elige un parámetro — no varios, para no mezclar unidades en el mismo eje — y varios sitios a la vez, cada uno como una línea de color distinto sobre el mismo eje Y. Los sitios no miden todos en el mismo instante exacto (cada uno hace su propio polling), así que el eje de tiempo se arma uniendo las marcas de todos los sitios elegidos, con `spanGaps` para no cortar las líneas en los huecos propios de cada uno — una limitación conocida a mejorar a futuro con un agrupado por intervalos redondeados.
- **Roadmap compartido**: se documentan en las páginas de Aire Saladillo y EMA Saladillo los 4 sensores/estaciones nuevos planeados (2 en Saladillo y 2 en 25 de Mayo, sumando una estación EMA en 25 de Mayo) — ver el roadmap de cada portal.
