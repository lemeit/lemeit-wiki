...
### Descargar todo el historial de un sensor

`GET /api/historico/:sensor_index` devuelve como máximo 20.000 filas por pedido (3000 por defecto si no se manda `limit`). Un sensor con varios meses de datos puede superar ese tope fácilmente — en ese caso no hay forma de traerlo todo en un solo pedido, hay que partirlo en tramos de fecha con `desde`/`hasta` (por ejemplo, uno por mes) y unir los CSV resultantes:
